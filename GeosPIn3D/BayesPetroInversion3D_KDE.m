function [Vpmap, Vsmap, Rhomap, Phimap, Claymap, Swmap, Time] = BayesPetroInversion3D_KDE(near, mid, far, TimeSeis, vpprior, vsprior, rhoprior, sigmaprior, elastrain, petrotrain, vpgrid, vsgrid, rhogrid, phigrid, claygrid, swgrid, sigmaerr, wavelet, theta, nv, h)

% BayesInversion3D_KDE computes the posterior distribution of 
% petrophysical properties using Bayesian linearized AVO inversion 
% (Buland and Omre, 2003), a non-parametric rock physics likelihood
...
%       sigmaprior = prior spatial covariance matrix (3*nm x 3 * nm)
...
%       h = kernel density smoothing parameter (reciprocal)
...
petrogrid = [phigrid claygrid swgrid];
elasgrid =[vpgrid vsgrid rhogrid];
hm(1) = (max(phigrid)-min(phigrid))/h;
hm(2) = (max(claygrid)-min(claygrid))/h;
hm(3) = (max(swgrid)-min(swgrid))/h;
hd(1) = (max(vpgrid)-min(vpgrid))/h;
hd(2) = (max(vsgrid)-min(vsgrid))/h;
hd(3) = (max(rhogrid)-min(rhogrid))/h;
[vpgrid,vsgrid,rhogrid]=ndgrid(vpgrid, vsgrid, rhogrid);
elasevalpoints = [vpgrid(:) vsgrid(:) rhogrid(:)];
...
for i=1:nxl
    ...
    for j=1:nil
        ...
        Pseis = zeros(size(elasevalpoints,1), nm);
        for k=1:nm
            Pseis(:,k)=mvnpdf(elasevalpoints, [vptrans(k) vstrans(k) rhotrans(k)], sigmatrans);
